fetch(
  ...
  .then(function (profile) {
    $(document).ready(function () {
      let heading = `<h1 class="text-center mt-4" id="heading"><a target="_blank" href="${profile.link}">${profile.name}</a></h1>`;
      ...
      for (let key in profile.projects) {
        let projectItems = `<li>
        <a href="${profile.projects[key].link}">${key}</a>: ${profile.projects[key].description}
      </li>`;
